...
# (cd ${ROOT_DIR}/deploy/samza && exec ./bin/run-job.sh --config-factory=org.apache.samza.config.factories.PropertiesConfigFactory --config-path=file://${ROOT_DIR}/build/yarn/$1/config/$1.properties)

(cd ${ROOT_DIR}/deploy/samza && exec ./bin/run-job.sh --config-factory=org.apache.samza.config.factories.PropertiesConfigFactory --config-path=file://${ROOT_DIR}/$1.properties)